...
if initial:
    ...
    if mynumcl>1:
        ...
        center,lab=kmeans2(numpy.array(trpos), mynumcl,iter=1000,minit='points')
        cfg.numcl=mynumcl
        cfg.fy=[cfg.fy[0]]*mynumcl
        cfg.fx=[cfg.fx[0]]*mynumcl
        hogpcl=lab
        hogncl=numpy.array(hogncl)
        #ratio=numpy.sum(hogpcl==1)/float(numpy.sum(hogpcl==0)+numpy.sum(hogpcl==1))
        #hogncl[:int(ratio*len(hogncl))]=1
        for l in range(cfg.numcl):
            hogncl[l*len(hogncl)/float(mynumcl):(l+1)*len(hogncl)/float(mynumcl)]=l
            print "# Cluster %d:"%l,numpy.sum(lab==l)
            lg.info("# Cluster %d:%d"%(l,numpy.sum(lab==l)))
        print "Elements ",numpy.sum(hogpcl==0),numpy.sum(hogpcl==1)
        #raw_input()
