...
if cfg.db=="track":
    try:
        ff=open(cfg.dbpath+"/videos/%s/%s_frames.txt"%(cfg.cls,cfg.cls))
        frm=ff.readline()
        sframe=int(frm.split(",")[0])
        eframe=int(frm.split(",")[1])
    except:
        sframe=1
...
if initial:
    ...
    #from scipy.ndimage import zoom
    for im in trPosImages: # for each image
        aim=util.myimread(im["name"])
        for mul in range(20):
    ...
    for idm,m in enumerate(models):   
        ...
        pl.imshow(imm)
        pylab.savefig("%s_hog%d_cl%d.png"%(testname,0,idm))
...
if not os.path.exists(path):
    os.mkdir(path)
ff=open("%s/detctions.txt"%(testname),"w")
ff.close()
...
for it in range(cpit,cfg.posit):
    ...
    if not cfg.checkpoint or not loadedchk:
        ...
        for ii,res in enumerate(itr):
            ...
            if localshow:
                ...
                #raw_input()
        if cfg.savetrack:
            pylab.savefig("%s/det%05d.png"%(testname,it))
            ff=open("%s/detctions.txt"%(testname),"a")
            ff.write("%f %f %f %f\n"%(lpdet[-1]["bbox"][0],lpdet[-1]["bbox"][1],lpdet[-1]["bbox"][2],lpdet[-1]["bbox"][3]))
            #ff.write("%f %f %f %f\n"%(lpdet[-1]["bbox"][0],im.shape[1]-lpdet[-1]["bbox"][1],lpdet[-1]["bbox"][2],im.shape[1]-lpdet[-1]["bbox"][3]))
    ...
    for nit in range(cfg.negit):
        ...
        #import pegasos   
        if cfg.useSGD:
            w,r,prloss=pegasos.trainCompSGD(trpos,trneg,"",trposcl,trnegcl,oldw=w,pc=cfg.svmc,k=numcore*2,numthr=numcore,eps=0.01,sizereg=sizereg,valreg=cfg.valreg,lb=cfg.lb,mintimes=1,maxtimes=20)#,notreg=notreg)
        else:
        ...
        for idm,m in enumerate(models[:cfg.numcl]):   
            ...
            pl.draw()
            pl.show()
            pylab.savefig("%s_def%dl_cl%d.png"%(testname,0,idm))
            ...
            pl.draw()
            pl.show()
            pylab.savefig("%s_def%dq_cl%d.png"%(testname,0,idm))
...
Negative in cache vectors %d
        ...
        if not(parallel):
            itr=itertools.imap(hardNegPosCache,arg)        
        else:
    ...
    #auxname=trPosImages[0]["name"]
    trPosImages=trPosImagesFull[it:it+1]
    #trPosImages[0]["name"]=auxname
#    if len(lpdet[-2]["bbox"])>1:
    #assuming that the detectior always detects
    trPosImages[0]["bbox"][0][:4]=lpdet[-1]["bbox"]
#    else:
#        trPosImages[0]["bbox"][0][:4]=
    if lpdet>cfg.memory:
        lpdet=lpdet[-cfg.memory:]
        lpfeat=lpfeat[-cfg.memory:]
        lpedge=lpedge[-cfg.memory:]
