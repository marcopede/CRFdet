...
if initial:
    ...
    for im in trPosImagesNoTrunc: # for each image
        ...
        for bb in im["bbox"]: # for each bbox (y1,x1,y2,x2)
            ...
            if cfg.usepose:
                if cfg.db=="MultiPIEramanan":
                    idm=abs(int(float(im["pose"])/91.0*cfg.numcl))
                    #print "Pose:",im["pose"]
                if cfg.db=="MultiPIE2":
                    idm=abs(int(float(im["pose"])/91.0*cfg.numcl))
                    #print "Pose:",im["pose"]
                    ...
                    if cfg.numcl==2:
                        if cfg.db=="LFW":
                            ...
                            idm=int(min(abs(im["pose"][0][2]/numpy.pi*4),1))
                    if cfg.numcl==3:
                        if cfg.db=="LFW":
                            idm=abs(im["pose"])/2
                        elif cfg.db=="AFLW":
                            idm=int(min(abs(im["pose"][0][2]/numpy.pi*6),2))
                    if cfg.numcl==4:
                        if cfg.db=="LFW":
                            error#idm=abs(im["pose"])/2
                        elif cfg.db=="AFLW":
                            #idm=int(min(abs(im["pose"][0][2]/numpy.pi*2*4),3))
                            idm=int(min((abs((im["pose"][0][2]/numpy.pi*2)))*4,cfg.numcl-1))
                    if cfg.numcl>4:
                        idm=int(min(numpy.sqrt(abs((im["pose"][0][2]/numpy.pi*2)))*cfg.numcl,cfg.numcl-1))
                print "------",im["id"],"-------",idm
            else:
    ...
    if cfg.useRL:
        for l in range(cfg.numcl):
            ...
            #rrnum=len(mytrpos)
            #rrnum=min(len(mytrpos)*3,500)#to avoid too long clustering
            rrnum=min(len(mytrpos)*3,3000)#to avoid too long clustering
            #rrnum=min(len(mytrpos),10)#to avoid too long clustering
            for rr in range(rrnum):
                print "Clustering iteration ",rr,"/",rrnum
    ...
    if not cfg.checkpoint or not loadedchk:
        ...
        for ii,res in enumerate(itr):
            ...
            if localshow:
                ...
                iname=arg[ii]["file"].split("/")[-1]
                if res[0]!=[]:
                    detectCRF.visualize2([res[0]],cfg.N,im,cbb,iname+"\n"+text,line=True)
                else:
                    detectCRF.visualize2([],cfg.N,im,cbb,iname+"\n"+text)
                if cfg.useFacial:
