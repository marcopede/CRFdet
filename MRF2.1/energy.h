...
public:
    ...
    typedef GraphType::gr_captype Value;
    typedef GraphType::gr_flowtype TotalValue;
...
inline Energy::Energy(void (*err_function)(const char *)) : GraphType(60,120,err_function) //remember to set the parameters here
{
    Econst = 0;
                              ...
                              Value C, Value D)
{
    ...
    */
    ...
    assert(B + C >= -0.00001); /* check regularity  changed because using float*/
    if (B < 0)
    {
        /* Write it as
           B B  +  -B 0  +  0   0
...
inline void Energy::ereset() { reset(); }
