...
void GCoptimization::initialize_memory()
{
    ...
    m_energies = NULL;
    m_solutions = NULL;
    m_variables = (Energy::Var *) new Energy::Var[m_nLabels];
    //m_energies     = (Energy *) new Energy[m_nLabels];
    ...
    {
        m_labelTable[i] = i;
        //m_energies[i] = new Energy();
    }
    srand(3);//fix random values to repeat results
}
...
/*GCoptimization::GCoptimization(PixelType width,PixelType height,int nLabels,EnergyFunction *eng,Energy *labeltable):MRF(width,height,nLabels,eng)
{
     commonGridInitialization(width,height,nLabels);
    ...
    if (labeltable!=NULL)
    {
        m_energies     = (Energy *) new Energy[m_nLabels]; //remember to delete it!!!
        
    }
}*/


//used to save the graphs for a new optimization
void GCoptimization::setEnergies(Energy* Energies)
{
    if (Energies==NULL)
    {
        //printf("Allocating Memory for %d labels.\n",m_nLabels);
        m_energies     = (Energy *) new Energy[m_nLabels];
        terminateOnError(!m_energies,"out of memory");
    }
    else //use from previous run
        m_energies = Energies;
}

Energy* GCoptimization::getEnergies()
{
    return m_energies;
}

//used to save the solutions for a new optimization
void GCoptimization::setSolutions(int* Solutions)
{
    if (Solutions==NULL)
    {
        //printf("Allocating Memory for %d labels.\n",m_nLabels);
        m_solutions     = new int[m_nPixels*m_nLabels];
        terminateOnError(!m_solutions,"out of memory");
    }
    else //use from previous run
        m_solutions = Solutions;
}

int* GCoptimization::getSolutions()
{
    return m_solutions;
}
...
void GCoptimization::setMyLabelOrder(int *myorder)
{
    m_random_label_order = 0;
    for ( int i = 0; i < m_nLabels; i++ )
        m_labelTable[i] = myorder[i];
}
...
GCoptimization::~GCoptimization()
{
    ...
    if (m_solutions!=NULL) delete [] m_solutions;
    delete [] m_variables;
    //printf("Descructor end\n");
}
...
void Expansion::perform_alpha_expansion(LabelType alpha_label)
{
    ...
    {
        e = &(m_energies[alpha_label]);
        e ->ereset();    
        //printf("Label %d\n",alpha_label);
    }

    if (m_solutions==NULL) //rebuild the trees from scracth
    {
    ...
    /*else // change only the parts of the trees that have changed
    {
        for ( i = 0; i < m_nPixels; i++ )
        {
            if ( m_labeling[i] != alpha_label )
        ...
        for ( i = 0; i < m_nPixels; i++ ) //compare old and new labeling
        {
            if (m_solutions[alpha_label*m_nPixels+i]!=m_labeling[i])
            {
                printf("Error, labeling has changed!!\n");
                //set node has changed
            }
        }
    ...
    if (m_solutions!=NULL)
    {
        for (i = 0;i < m_nPixels; i++ )
        {
            m_solutions[alpha_label*m_nPixels+i]=m_labeling[i];
        }
    }
...
/**********************************************************************************************/
/* Performs alpha-expansion for  regular grid graph for case when energy terms are         */
/* specified by a function */
...
GCoptimization::EnergyType Expansion::oneExpansionIteration()
{
    ...
    if (m_random_label_order) scramble_label_table();
    //use the right oreder only the frist time
    //if (m_random_label_order==0) 
    //    m_random_label_order = 1;
